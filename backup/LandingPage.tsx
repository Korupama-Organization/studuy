...
const LandingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const scrollableParent = target.closest('.slide-scrollable') as HTMLElement;
  ...
  const TestimonialsSectionLocal = () => (
  ...
  const slideComponents = [
    ...
    <AIAnalysisSection key="aiAnalysis" />,
    <ExperienceSection key="experience" />,
    <ValueSection key="value" />,
    <JobConnectionSection key="jobConnection" />,
    <RecruiterSection key="recruiter" />,
    <TestimonialsSectionLocal key="testimonials" />,
    <CTASection key="cta" />,
    <FAQSection key="faq" />,
        ...
        <div className="max-w-[1280px] mx-auto px-4 lg:px-0">
          <div className="flex justify-between items-center h-full">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-lg" style={{ background: 'linear-gradient(135deg, #4D55CC 0%, #8B4CFF 100%)' }}></div>
      ...
      {isDesktop ? (
        <div className="h-[calc(100vh-5rem)] w-full overflow-hidden relative">
          <motion.div
            ...
            style={{ height: `${numSlides * 100}%` }}
            animate={{ y: `-${(currentSlide / numSlides) * 100}%` }}
            transition={{ type: 'spring', stiffness: 300, damping: 30 }}
          >
            {slideComponents.map((slide, index) => (
              <div
                key={index}
                className="w-full slide-scrollable overflow-y-auto"
                style={{ height: `${100 / numSlides}%` }}
              >
                <motion.div
